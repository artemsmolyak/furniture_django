...
class OrderForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
                   'fio': forms.TextInput(attrs={'size': 37, "class" : "ajsdfjasdfj"}),

                   'nameOrder': forms.TextInput(attrs={'size': 37}),
                   'nameContract': forms.TextInput(attrs={'size': 37}),

                   'address': forms.TextInput(attrs={'size': 37}),
                   'phone': forms.TextInput(attrs={'size': 11}),

                   'trelloLink': forms.TextInput(attrs={'size': 37}),

                   'details': forms.Textarea(attrs={'cols': 37, 'rows': 4 })
                   }
